...
func (s *GameScene) updateCamera() {
	...
	if s.state.InputHandler.ActionIsPressed(gamestate.ActionDown) || (y > s.Height-20 && y <= s.Height) {
		s.target[1] += 2
	}

	if s.state.InputHandler.ActionIsPressed(gamestate.ActionUp) || (y < 20 && y >= 0) {
		s.target[1] -= 2
	}

	if s.state.InputHandler.ActionIsPressed(gamestate.ActionRight) || (x > s.Width-20 && x <= s.Width) {
		s.target[0] += 2
	}

	if s.state.InputHandler.ActionIsPressed(gamestate.ActionLeft) || (x < 20 && x >= 0) {
		s.target[0] -= 2
	}
...
func (s *GameScene) Draw(screen *ebiten.Image) {
	...
	ebitenutil.DebugPrintAt(s.worldScreen, s.state.T("draw_here"), 200, 200)
...
func (s *GameScene) Load(state *gamestate.GameState, manager stagehand.SceneController[*gamestate.GameState]) {
